...
from Helper_Functions import Gen_Secret, Hash256
...
def main():
    ...
    # LN transactions ##################################################################################################
    random = Gen_Secret()
    tx_stateLN = Gen_state_LN(tx_state_in, id_s0, id_r0, a, balLeft, balRight, fee, Hash256(random), t)
    Print_TX(tx_stateLN, 'tx_stateLN')
    tx_LN_pay = Gen_tx_LN_pay(TxInput(tx_stateLN.get_hash(), 0), id_r0, id_s0, a, 2 * fee, random, Hash256(random), t)
    Print_TX(tx_LN_pay, 'tx_LN_pay')
    tx_LN_refund = Gen_tx_LN_refund(TxInput(tx_stateLN.get_hash(), 0), id_s0, id_r0, a, 2 * fee)
...
# Lightning:
def Gen_state_LN(tx_in: TxInput, id_s: Id, id_r: Id, a: float, x_left: float, x_right: float,
                 fee: float, hash_secret, t: int) -> Transaction:
    r_sc = Script(['OP_IF',
                   'OP_2', id_s.pk.to_hex(), id_r.pk.to_hex(), 'OP_2', 'OP_CHECKMULTISIGVERIFY',
                   t, 'OP_CHECKLOCKTIMEVERIFY',
                   'OP_ELSE',
                   'OP_2', id_s.pk.to_hex(), id_r.pk.to_hex(), 'OP_2', 'OP_CHECKMULTISIGVERIFY',
                   'OP_HASH256', hash_secret, 'OP_EQUALVERIFY',
                   'OP_ENDIF'])

    tx_out0 = TxOutput(a, r_sc.to_p2sh_script_pub_key())
    tx_out1 = TxOutput(x_left - a - fee, id_s.p2pkh)
...
def Gen_tx_LN_pay(tx_in_state: TxInput, id_r: Id, id_s: Id, a: float,
                  fee: float, secret, hash_secret, t: int) -> Transaction:
    tx_out0 = TxOutput(a - fee, id_r.p2pkh)
    ...
    r_sc = Script(['OP_IF',
                   ...
                   'OP_HASH256', hash_secret, 'OP_EQUALVERIFY',
                   'OP_ENDIF'])
    unlock_script = [0x00, secret, id_r.sk.sign_input(tx, 0, r_sc), id_s.sk.sign_input(tx, 0, r_sc), r_sc.to_hex()]
    tx_in_state.script_sig = Script(unlock_script)

    return tx


def Gen_tx_LN_refund(tx_in_state: TxInput, id_s: Id, id_r: Id, a: float, fee: float) -> Transaction:
    # tx_in must hold at least n times eps coins plus a fee
    tx_out0 = TxOutput(a - fee, id_s.p2pkh)
